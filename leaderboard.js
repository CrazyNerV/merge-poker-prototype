...
async function loadRanking(){stageTitle.textContent=currentStage;stageSummary.textContent=`STAGE ${currentStage}`;showState('랭킹을 불러오는 중…');refreshBtn.disabled=true;try{if(!leaderboardApi)throw new Error('백엔드 설정을 찾을 수 없습니다.');const{data,error}=await leaderboardApi.from('leaderboard').select('player_name,score,play_time').eq('stage',currentStage).order('score',{ascending:false}).order('play_time',{ascending:true}).limit(100);if(error)throw error;list.innerHTML='';data.forEach((entry,index)=>list.appendChild(rankingItem(entry,index)));if(data.length){state.className='ranking-state hidden'}else{showState('아직 등록된 기록이 없습니다.')}}catch(error){showState('리더보드를 불러오지 못했습니다. 잠시 후 다시 시도해 주세요.',true)}finally{refreshBtn.disabled=false}}
