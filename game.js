...
function showLeaderboardResult(kicker,title,reason,result,entries){showModal(kicker,title,`<p>${reason}</p><div class="score-formula"><span>HAND ${sliceScore.toLocaleString()}</span><span>SPIN ${spinsLeft} × 1,000</span><span>TIME ${result.elapsed}s × 10</span><strong>FINAL ${result.finalScore.toLocaleString()}</strong></div><h2>STAGE ${selectedStage} · ${leaderboardApi?'GLOBAL':'LOCAL'} TOP 100</h2>${leaderboardHtml(entries)}`,'단계 선택',showStageSelect)}
async function finishGame(kicker,title,reason){
  ...
  showModal(kicker,title,`<p>${reason}</p><div class="score-formula"><span>HAND ${sliceScore.toLocaleString()}</span><span>SPIN ${spinsLeft} × 1,000</span><span>TIME ${result.elapsed}s × 10</span><strong>FINAL ${result.finalScore.toLocaleString()}</strong></div><label class="name-entry">STAGE ${selectedStage} TOP 100 기록 이름<input id="player-name" maxlength="16" autocomplete="nickname" placeholder="PLAYER"></label>`,'기록 저장',async()=>{const input=document.querySelector('#player-name'),name=input?.value.trim()||'PLAYER',record={name,score:result.finalScore,sliceScore,spinsLeft,elapsed:result.elapsed,stage:selectedStage,date:new Date().toISOString()};const button=document.querySelector('#modal-btn');button.disabled=true;button.textContent='저장 중…';let latest;try{latest=await persistScore(record,entries)}catch{entries.push(record);entries.sort((a,b)=>b.score-a.score||a.elapsed-b.elapsed);saveLocalLeaderboard(entries);latest=entries.slice(0,100);showToast('서버 저장 실패 · 로컬 저장')}showLeaderboardResult(kicker,title,reason,result,latest)})
}
let tutorialPage=0;
...
function showStageSelect(){gameplayActive=false;const options=Array.from({length:7},(_,index)=>{const stage=index+1;return `<button class="stage-option" type="button" data-stage="${stage}"><span>STAGE ${stage}</span><small>FIX ${stage}</small><strong>SPIN ${STAGE_SPINS[stage]}</strong></button>`}).join('');showModal('SELECT STAGE','난이도 선택',`<p class="stage-select-copy">FIX 블록 수와 시작 SPIN 기회를 선택합니다.</p><div class="stage-select">${options}</div>`,null,null);document.querySelectorAll('.stage-option').forEach(button=>button.onclick=()=>{selectedStage=Number(button.dataset.stage);reset();openHelp()})}
