...
const ROWS=7,COLS=5,STARTING_SPINS=10,HAND_SCORE={'One Pair':100,'Two Pair':250,'Three of a Kind':400,'Straight':700,'Full House':1000,'Four of a Kind':1600,'Five of a Kind':3000};
let grid=[],fix=[],score=0,sliceScore=0,spins=0,spinsLeft=STARTING_SPINS,drag=null,hands=[],pointerStart=null,pointerRow=null,chain=0,busy=false,startedAt=null,gameEnded=false;
...
function reset(){grid=Array.from({length:4},()=>Array(ROWS).fill(null));fix=Array.from({length:ROWS},()=>({suit:Math.floor(Math.random()*4),rank:weightedRank()}));score=0;sliceScore=0;spins=0;spinsLeft=STARTING_SPINS;drag=null;hands=[];chain=0;busy=false;startedAt=null;gameEnded=false;closeModal();render()}
...
function mergeTo(row,col){if(!drag||busy)return;const from=getCard(drag.row,drag.col),target=getCard(row,col),source={...drag};drag=null;if(col===4&&target?.rank===8){showToast('FIX 9는 Merge할 수 없어요');render();return}if(!from||!target||source.col===col&&source.row===row||from.suit!==target.suit||from.rank!==target.rank){showToast('동일한 무늬와 Rank만 Merge할 수 있어요');render();return}if(col<4&&col!==source.col){showToast('Production 카드는 같은 Line 안에서 Merge하세요');render();return}if(from.rank===8){grid[source.col][source.row]=null;if(col<4)grid[col][row]=null;else{fix[row]=null;gravityFix()}score+=2500;gravity(source.col);if(col<4)gravity(col);flash('MAX MERGE');showToast('9 + 9 Max Merge!');render();return}if(col===4)fix[row].rank++;else grid[col][row].rank++;grid[source.col][source.row]=null;score+=50*(from.rank+1);gravity(source.col);chain=0;flash(`${RANKS[from.rank+1]} MERGE`);render()}
...
function clearHand(row){const hand=hands.find(h=>h.row===row);if(!hand)return;chain++;for(let col=0;col<4;col++){grid[col][row]=null;gravity(col)}fix[row]=null;gravityFix();const gain=HAND_SCORE[hand.name]*chain;score+=gain;sliceScore+=gain;const spinBonus=true;spinsLeft++;flash(`${hand.name.toUpperCase()} · ${chain} CHAIN`);showToast(`${hand.name} +${gain.toLocaleString()}${spinBonus?' · SPIN +1':''}`);render()}
...
function tutorialHtml(page){
  ...
  if(page===1)return `<div class="tutorial-page"><div class="tutorial-board">${tutorialRow([3,3,3,3,3])}<div class="tutorial-hand-ready">FIVE OF A KIND · READY</div></div><h2>포커 핸드와 슬라이스 점수</h2><p>가로 5장이 아래 포커 핸드 중 하나가 되면 해당 행이 빛납니다.</p><div class="tutorial-score-list">${Object.entries(HAND_SCORE).map(([name,value])=>`<span>${name}<strong>+${value.toLocaleString()}</strong></span>`).join('')}</div></div>`;
  return `<div class="tutorial-page"><div class="tutorial-board submit-board">${tutorialRow([2,3,4,5,6])}<div class="tutorial-swipe"><i>→</i><span>SWIPE</span></div></div><h2>완성된 핸드 제출하기</h2><p>빛나는 행을 좌우로 스와이프하면 다섯 카드와 해당 FIX 블록이 함께 소거됩니다.</p><div class="tutorial-rewards"><span>FIX <strong>−1</strong></span><span>SPIN <strong>+1</strong></span></div><p class="tutorial-note">FIX 라인을 모두 비우면 Stage Clear!</p></div>`;
}
