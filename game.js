...
function gemHtml(suit,rank,extra=''){return `<div class="gem ${extra}" style="--gem-x:${(rank+1)*100/9}%;--gem-y:${suit*100/3}%" role="img" aria-label="${['Ruby','Sapphire','Emerald','Amethyst'][suit]} Lv.${rank+1}"></div>`}
function render(){detectHands();board.innerHTML='';for(let row=0;row<ROWS;row++){for(let col=0;col<COLS;col++){const card=getCard(row,col),cell=document.createElement('div');cell.className=`cell ${col===4?'fix-cell':''} ${hands.some(h=>h.row===row)?'hand':''}`;cell.dataset.row=row;cell.dataset.col=col;if(card){const suit=SUITS[card.suit];cell.innerHTML=`<div class="card gem-card ${suit.red?'red':''} ${SUIT_CLASS[card.suit]} ${col===4?'fix-card':''}" draggable="${col<4}">${gemHtml(card.suit,card.rank)}${col===4?'<div class="fix-tag">FIXED</div>':''}</div>`}bindCell(cell,row,col);board.appendChild(cell)}}renderHandStatus();document.querySelector('#score').textContent=score.toLocaleString();document.querySelector('#spin-count').textContent=spinsLeft;document.querySelector('#fix-count').textContent=fix.filter(Boolean).length;document.querySelector('#progress-fill').style.width=`${(ROWS-fix.filter(Boolean).length)/ROWS*100}%`;spinBtn.disabled=busy||!canSpin();renderReels();if(fix.every(card=>card===null))stageClear();else if(!canSpin()&&!hasReliefMerge())gameOver()}
function renderReels(results){document.querySelector('#reels').innerHTML=SUITS.map((s,i)=>`<div class="reel ${s.red?'red':''} ${SUIT_CLASS[i]} ${results?'rolling':''}">${gemHtml(i,results??0,'reel-gem')}<small>${['RUBY','SAPPHIRE','EMERALD','AMETHYST'][i]}</small></div>`).join('')}
...
let tutorialPage=0;
function tutorialCard(rank,suit,fixCard=false){const data=SUITS[suit];return `<div class="tutorial-cell ${fixCard?'tutorial-fix':''}"><div class="card gem-card ${data.red?'red':''} ${SUIT_CLASS[suit]} ${fixCard?'fix-card':''}">${gemHtml(suit,Number(rank)-1)}${fixCard?'<div class="fix-tag">FIXED</div>':''}</div></div>`}
function tutorialRow(ranks){return `<div class="tutorial-column-heads"><span>R</span><span>S</span><span>E</span><span>A</span><b>FIX</b></div><div class="tutorial-row">${ranks.map((rank,index)=>tutorialCard(rank,index<4?index:0,index===4)).join('')}</div>`}
